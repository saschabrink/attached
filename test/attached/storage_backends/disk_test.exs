defmodule Attached.StorageBackends.DiskTest do
  ...
  # The :local Disk instance's config, as the facade would pass it.
  defp disk_config do
    {Attached.StorageBackends.Disk, config} = Attached.StorageBackends.resolve!(:local)
    config
  end
  ...
  describe "path_for/2 security" do
    test "raises on blank key" do
      assert_raise ArgumentError, ~r/blank/, fn ->
        Attached.StorageBackends.Disk.path_for(disk_config(), "")
      end
    end

    test "raises on nil key" do
      assert_raise ArgumentError, ~r/blank/, fn ->
        Attached.StorageBackends.Disk.path_for(disk_config(), nil)
      end
    end

    test "raises on dot-dot traversal" do
      assert_raise ArgumentError, ~r/traversal/, fn ->
        Attached.StorageBackends.Disk.path_for(disk_config(), "../../etc/passwd")
      end
    end

    test "raises on single-dot segment" do
      assert_raise ArgumentError, ~r/traversal/, fn ->
        Attached.StorageBackends.Disk.path_for(disk_config(), "./sneaky")
      end
    end

    test "raises on null byte" do
      assert_raise ArgumentError, ~r/null byte/, fn ->
        Attached.StorageBackends.Disk.path_for(disk_config(), "evil\0key")
      end
    end

    test "accepts a normal key" do
      path = Attached.StorageBackends.Disk.path_for(disk_config(), "abcdef1234")
      assert path =~ "/ab/cd/abcdef1234"
    end
  end

  describe "path_for/2 sharding" do
    test "first two chars become first shard, next two become second" do
      key = "abcdef1234"
      assert Attached.StorageBackends.Disk.path_for(disk_config(), key) =~ "/ab/cd/#{key}"
    end

    test "matches Active Storage two-level layout" do
      key = "xtapjjcjiudrlk3tmwyjgpuobabd"
      assert Attached.StorageBackends.Disk.path_for(disk_config(), key) =~ "/xt/ap/#{key}"
    end

    test "variant keys are sharded on their prefix" do
      key = "variants/someuuid/digest"
      assert Attached.StorageBackends.Disk.path_for(disk_config(), key) =~ "/va/ri/#{key}"
    end

    test "_variants/ namespace shards on the parent key and doesn't repeat the prefix" do
      key = "_variants/abcdef1234-thumb-aaaa"
      path = Attached.StorageBackends.Disk.path_for(disk_config(), key)
    ...
    test "uploaded file lands under sharded path", %{upload: upload} do
      ...
      expected_path = Attached.StorageBackends.Disk.path_for(disk_config(), original.key)
  ...
  describe "download_chunk/3" do
    test "returns the requested byte range", %{upload: upload} do
      ...
      assert {:ok, "hello"} = Attached.StorageBackends.Disk.download_chunk(disk_config(), original.key, 0..4)
    end
    ...
    test "returns a middle range", %{upload: upload} do
      ...
      assert {:ok, "world"} = Attached.StorageBackends.Disk.download_chunk(disk_config(), original.key, 6..10)
    end

    test "returns error for a missing key" do
      assert {:error, :not_found} = Attached.StorageBackends.Disk.download_chunk(disk_config(), "nonexistent", 0..4)
    end
  end
  ...
  describe "compose/3" do
    test "concatenates source files into destination" do
      ...
      :ok = Attached.StorageBackends.Disk.upload(disk_config(), key1, write_tmp("hello "))
      :ok = Attached.StorageBackends.Disk.upload(disk_config(), key2, write_tmp("world"))
      :ok = Attached.StorageBackends.Disk.compose(disk_config(), [key1, key2], dest)

      assert {:ok, "hello world"} = Attached.StorageBackends.Disk.download(disk_config(), dest)
    after
